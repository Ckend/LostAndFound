...
<template>
    <div style="text-align:center;padding-bottom: 50px">
        <div class="demo-table-settings">
            <h2 style="padding-top: 10px" >{{selectSchool}} - 已有失物</h2>
            <br>
        ...
        <br>
        <!--client-->
        <Table ref="table" v-if="switchTabCard === true && switchTabNoCard === false" :columns="client"
               :loading="this.loading" :data="Cards"></Table>
        <Table ref="table" v-else-if="switchTabNoCard === true && switchTabCard === false" :columns="client"
               :loading="this.loading" :data="NoCards"></Table>
        <Table ref="table" v-else-if="((switchTabNoCard === true && switchTabCard === true)||
                (switchTabNoCard === false && switchTabCard === false))" :columns="client"
        ...
        methods: {
            ...
            search_data () {
                if (this.search !== ''){
                    ...
                    this.axios({
                        method: 'get',
                        url: '/api/search/'+this.search+'/'+(this.page-1)*10+'/10/'+this.selectSchool,
                    }).then(function (res) {
            ...
            get_data () {
                ...
                this.axios({
                    method: 'get',
                    url: '/api/getLostData/'+(this.page-1)*10+'/10/'+this.selectSchool
                }).then(function (res) {
                    that.total = res.data[1]
                    that.manageData = res.data[0]
                    for (let x in that.manageData) {
